...
export function PatientDataPage(): ReactNode {
  ...
  const [activeTab, setActiveTab] = useState<
    "sessions" | "upcoming" | "base-history" | "patient-data"
  >("sessions");
      ...
      <header className="shrink-0 p-4 bg-white border-b border-border/40 shadow-sm z-20">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2 overflow-x-auto pb-1 md:pb-0 no-scrollbar">
            <Button
              ...
              className="rounded-sm font-bold border-primary/20 text-primary hover:bg-primary/5 h-10 whitespace-nowrap"
              icon={<Plus size={16} />}
            >
              Nueva Evolución
            </Button>
        ...
        <div className="flex items-center gap-1 p-1 bg-muted/40 rounded-md w-full md:w-fit mt-6 border border-border/40 overflow-x-auto no-scrollbar">
          <button
            ...
            className={`flex items-center gap-2 px-5 py-2 rounded-sm text-[11px] font-black uppercase tracking-widest transition-all whitespace-nowrap ${activeTab === "sessions" ? "bg-primary text-white shadow-md shadow-primary/20" : "text-muted-foreground hover:bg-white/60"}`}
          >
            <Clock size={14} /> Historial
          </button>
          <button
            onClick={() => setActiveTab("upcoming")}
            className={`flex items-center gap-2 px-5 py-2 rounded-sm text-[11px] font-black uppercase tracking-widest transition-all whitespace-nowrap ${activeTab === "upcoming" ? "bg-primary text-white shadow-md shadow-primary/20" : "text-muted-foreground hover:bg-white/60"}`}
          >
            <CalendarDays size={14} /> Próximas Citas
          </button>
          <button
            onClick={() => setActiveTab("base-history")}
            className={`flex items-center gap-2 px-5 py-2 rounded-sm text-[11px] font-black uppercase tracking-widest transition-all whitespace-nowrap ${activeTab === "base-history" ? "bg-primary text-white shadow-md shadow-primary/20" : "text-muted-foreground hover:bg-white/60"}`}
          >
            <ClipboardList size={14} /> Historia Base
          </button>
          <button
            onClick={() => setActiveTab("patient-data")}
            className={`flex items-center gap-2 px-5 py-2 rounded-sm text-[11px] font-black uppercase tracking-widest transition-all whitespace-nowrap ${activeTab === "patient-data" ? "bg-primary text-white shadow-md shadow-primary/20" : "text-muted-foreground hover:bg-white/60"}`}
          >
        ...
        <div className="max-w-6xl mx-auto p-4 md:p-8">
          ...
          {activeTab === "sessions" && (
            <div className="animate-in fade-in slide-in-from-bottom-2 duration-500 pb-10">
              {isAddingNote && (
                <div className="mb-8 p-6 bg-white rounded-md border-2 border-primary/20 shadow-xl animate-in zoom-in-95">
                    ...
                    <div className="space-y-4">
                      <div>
                        ...
                        <select
                          ...
                          className="w-full bg-muted/30 border border-border/40 rounded-sm px-4 py-3 text-xs font-bold focus:ring-2 focus:ring-primary/20 outline-none"
                        >
                      ...
                      <div>
                        ...
                        <textarea
                          ...
                          className="w-full h-32 bg-muted/30 border border-border/40 rounded-sm px-4 py-3 text-xs font-bold focus:ring-2 focus:ring-primary/20 outline-none resize-none"
                        />
                      </div>
                    </div>
                    <div>
                      ...
                      <textarea
                        ...
                        className="w-full h-44 bg-muted/30 border border-border/40 rounded-sm px-4 py-3 text-xs font-bold focus:ring-2 focus:ring-primary/20 outline-none resize-none"
                      />
                    </div>
                  </div>
                  <div className="flex justify-end gap-3 mt-6 pt-6 border-t border-border/40">
                    ...
                    <Button
                      ...
                      className="rounded-sm font-bold gradient-primary px-8"
                      icon={<Save size={16} />}
                    >
                      Guardar Entrada
                    </Button>
            ...
            <div className="animate-in fade-in slide-in-from-bottom-2 duration-500 space-y-8 pb-10">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4 border-l-4 border-primary pl-4">
                ...
                <Button
                  variant="outline"
                  size="sm"
                  className="rounded-sm font-bold border-border/60 bg-white"
                >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <section className="bg-white p-6 rounded-md border border-border/40 shadow-sm space-y-4">
                ...
                <section className="bg-white p-6 rounded-md border border-border/40 shadow-sm space-y-4 lg:col-span-2">
                  <h4 className="flex items-center gap-2 text-[10px] font-black text-muted-foreground uppercase tracking-widest border-b border-border/20 pb-3">
                ...
                {/* Alertas Críticas */}
                <section className="bg-destructive/5 p-6 rounded-md border border-dashed border-destructive/20 lg:col-span-1 flex flex-col justify-center">
                  <div className="flex items-center gap-4">
                    <ShieldAlert
                      size={32}
          ...
          {activeTab === "patient-data" && (
            <div className="animate-in fade-in slide-in-from-bottom-2 duration-500 space-y-8 pb-10">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-l-4 border-primary pl-4">
                ...
                <div className="flex gap-2">
                  <Button
                    variant="outline"
                    size="sm"
                    className="rounded-sm font-bold h-10 border-border/60 bg-white"
                  >
                    <Edit size={14} className="mr-2" /> Editar Perfil
                  </Button>
                  <Button className="rounded-sm font-bold h-10 bg-[#25D366] text-white hover:bg-[#128C7E] border-none">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {/* Identidad */}
                <div className="bg-white p-6 rounded-md border border-border/40 space-y-4">
                ...
                <div className="bg-white p-6 rounded-md border border-border/40 space-y-4">
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <span className="px-2 py-0.5 bg-primary/10 text-primary text-[10px] font-black rounded-sm">
                        {patient.bloodType}
                      </span>
                    </div>
                    <div>
              ...
              {/* Dirección Completa */}
              <div className="bg-white p-6 rounded-md border border-border/40 flex items-center gap-4">
                <div className="w-12 h-12 rounded-sm bg-muted flex items-center justify-center text-primary shadow-inner">
                  <MapPin size={24} />
                </div>
                <div>
