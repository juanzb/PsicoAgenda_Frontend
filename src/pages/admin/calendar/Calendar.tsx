...
export function CalendarPage(): ReactNode {
  ...
  const events: ICalendarEvent[] = useMemo(() => {
    return appointments.map(appt => {
      const patient = mockStorage.getPatients().find(p => p.id === appt.patientId);
  ...
  const shouldShowAll = currentUser.role === "ADMIN" || (currentUser.role === "DOCTOR" && isViewAllParam);

  const filteredEvents = useMemo(() => {
    if (!shouldShowAll) return events.filter(e => e.doctorId === currentUser.id);
    if (selectedDoctorId !== "ALL") return events.filter(e => e.doctorId === selectedDoctorId);
    return events;
    ...
    <div className="h-full flex flex-col gap-4 animate-in fade-in duration-500 p-1">
      <div className="flex flex-col md:flex-row items-center justify-between gap-4 p-4 bg-white rounded-lg border border-border/40 shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-bold text-foreground">Agenda de Citas</h3>
            <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">Gestiona tus consultas y horarios</p>
          </div>
        </div>
